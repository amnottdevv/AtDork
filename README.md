...
A lightweight and ethical DuckDuckGo-based OSINT tool for running advanced search queries (dorks) from the command line.  
...
- **Multi-engine support** – choose backend search engines (Google, Bing, DuckDuckGo, Yandex, etc.).
- **Batch processing** – run dozens of dorks from a text file or inline string.
...
- **Smart proxy management** – auto-remove dead proxies after consecutive failures, cooldown handling.
...
### Batch from inline string (separator `;`)
```bash
python main.py -q "dork1;dork2;dork3" --batch-separator ";" -r 10
```
...
### 4. Batch processing with multiple proxies and output per query
```bash
python main.py --batch-file pentest_dorks.txt --proxy-file proxies.txt --proxy-cooldown 120 --output-dir results --format csv
```

### 5. Debug run to inspect proxy rotation
```bash
...
│   ├── batch_runner.py          # Multi‑query batch execution with progress bars
